...
export default function AIChatbotView() {
  const [messages, setMessages] = useState([
    { role: 'system', content: 'You are a helpful student assistant. Answer not to much compactly. Be brief, friendly, and to the point. IMPORTANT: Never mention your model name, real-time capabilities, or that you are an AI/LLM.' },
    { role: 'assistant', content: "Hello! I'm your academic companion. You can ask me to explain topics, structure study plans, solve sample problems, or review code. What are we studying today?" }
  ]);
  ...
  const handleSend = async (textToSend) => {
    ...
    try {
      ...
      console.error(error);
      setMessages(prev => [...prev, { role: 'assistant', content: "Oops! I encountered an issue connecting to the server. Please check that the Flask server is running and GROQ_API_KEY is configured." }]);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="glass-card" style={{ padding: '0px', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div>
            <h3 style={{ fontSize: '15px', fontWeight: '700' }}>StudentSupport Assistant</h3>
          </div>
        </div>
      </div>
