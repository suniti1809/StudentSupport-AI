...
def chat_endpoint():
    ...
    if mode == "mock_interview":
        ...
        system_content = (
            ...
            "IMPORTANT: Never mention your model name, real-time capabilities, or that you are an AI/LLM."
        )
        ...
        system_content = (
            "You are a helpful student assistant. Answer not to much compactly. "
    ...
    for model_name in models_to_try:
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"[GROQ ERROR] Model {model_name} returned {response.status_code}: {response.text}")
                continue  # try the next model
        except Exception as e:
            print(f"[GROQ ERROR] Model {model_name} exception: {str(e)}")
            continue  # try the next model
    
    return jsonify({"reply": "Error: All AI models failed. Please try again in a moment (rate limit may apply)."}), 500
